...
При построении виджета все слайды указываются как массив в поле `\bestyii\swiper\Swiper::$items`.
...
> Заметка: При создании слайда без контента, поле `\bestyii\swiper\Slide::$content` будет иметь значение `null`
...
* Через алиас `\bestyii\swiper\Slide::BACKGROUND` - в данном случае 
...
* Через алиас `\bestyii\swiper\Slide::HASH`
* Через `\bestyii\swiper\Slide::$options['data']['hash']` напрямую
...
Все опции, указанные в `\bestyii\swiper\Slide::$options` будут переданы в опции тега.
...
### Настройка объектов `\bestyii\swiper\Slide`

При передаче в виджет в качестве слайда экземпляра объекта `\bestyii\swiper\Slide`
...
echo Swiper::widget( [
   ...
   * так как он имеет тип '\bestyii\swiper\Slide'
   */
  'itemOptions' => [
    'options' => [
      'tag'   => 'a',
